...
async def test_get_client_id_with_standard_jwt(
    ...
    with patch("waldiez_runner.dependencies.getters.ClientService", mock_client_service):
        ...
        with patch("waldiez_runner.dependencies.getters.app_state", mock_app_state):
            # Test
            result = await dependency_fn(credentials, session, None, context)
...
async def test_get_client_id_with_external_token(
    ...
    with patch("waldiez_runner.dependencies.getters.ClientService", mock_client_service):
        ...
        # Create needed parameters
        credentials = HTTPAuthorizationCredentials(scheme="Bearer", credentials="external-token")
        session = AsyncMock()
        context = RequestContext()
        ...
        with patch("waldiez_runner.dependencies.getters.app_state", mock_app_state):
            ...
            assert context.external_user_info == {"id": "ext-user", "name": "External User"}
...
async def test_get_client_id_both_auth_methods_fail(
    ...
    ext_exception = HTTPException(status_code=401, detail="Invalid external token")
    mock_verify_external.return_value = (None, ext_exception)
    
    with patch("waldiez_runner.dependencies.getters.ClientService", AsyncMock()):
        # Create the dependency function
        dependency_fn = get_client_id("tasks-api", allow_external_auth=True)
        ...
        with patch("waldiez_runner.dependencies.getters.app_state", mock_app_state):
            ...
            with pytest.raises(HTTPException) as excinfo:
                await dependency_fn(credentials, session, None, context)
